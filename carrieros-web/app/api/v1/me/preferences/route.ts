// GET/PATCH /api/v1/me/preferences — reads/writes the caller's OWN profile only. Transport only.
...
import { UpdatePreferencesBodySchema, OkResponseSchema, PreferencesResponseSchema } from '@/server/contract/schemas'
...
export async function GET(request: NextRequest) {
  ...
  const result = await createProfilePreferencesService(authed.supabase).getPreferences(actor.value)
  if (!result.ok) {
    logError({ route: 'api/v1/me/preferences GET', requestId, userId: authed.user.id }, result.error.detail)
    return domainErrorResponse(result.error)
  }
  return NextResponse.json(PreferencesResponseSchema.parse(result.value))
}
