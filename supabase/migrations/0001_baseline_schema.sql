...
-- REVIEWED BASELINE — the schema as it stood on 2026-07-26, captured verbatim
-- from supabase/schema/schema.sql (the file that was, until this commit, the
-- schema authority). Nothing was added, removed, reordered, or "cleaned up"
-- while capturing it: a baseline whose content differs from the thing it
-- baselines is not a baseline. Every known defect in here is therefore
-- preserved as-is and fixed forward in a later numbered migration, so that an
-- existing environment and a freshly-created one converge on the same state.
--
-- Deliberately NOT folded into this baseline (each gets its own migration, per
-- "do not combine unrelated destructive changes into the baseline"):
--   * SECTION 8c's blanket `GRANT ... ON ALL TABLES IN SCHEMA public TO
--     authenticated, service_role` plus permissive DEFAULT PRIVILEGES, which
--     auto-exposes every future table before anyone writes an RLS policy for
--     it. Tightening this is a behaviour change and is handled separately.
--   * Tables lacking a tenant-ownership column.
--   * Any status-vocabulary rework.
--
-- Applying this file to an EXISTING database is not supported and not needed:
-- existing environments are baselined by recording 0000/0001 as already-applied
-- (`node scripts/db/migrate.mjs --baseline`), which is exactly what the
-- "upgrade an existing environment" path in architecture/database-migrations.md
-- describes. This file is for creating a new database from nothing.
